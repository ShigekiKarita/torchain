...
test_sets="ntt2011n_headset_dev ntt2011n_headset_evl ntt2011n_arrayA01_dev ntt2011n_arrayA01_evl"
gmm=tri4
nj=400
...
set -euo pipefail
[ $# -ne 0 ] && log "Usage: $0" && exit 1

gmm_dir=exp/${gmm}

for f in data/${train_set}/feats.scp ${gmm_dir}/final.mdl; do
  if [ ! -f $f ]; then
    log "$0: expected file $f to exist"
    exit 1
  fi
done
...
if [ $stage -le 1 -a $stop_stage -ge 1 ]; then
  ...
  log "$0: preparing directory for low-resolution speed-perturbed data (for alignment)"
  utils/data/perturb_data_dir_speed_3way.sh data/${train_set} data/${train_set}_sp
  log "$0: making MFCC featuresfor low-resolution speed-perturbed data"
...
if [ $stage -le 2 -a $stop_stage -ge 2 ]; then
  ...
  log "$0: creating high-resolution MFCC features"
...
if [ $stage -le 3 -a $stop_stage -ge 3 ]; then
  ...
  log "$0: computing a PCA transform from the hires data."
  ...
  log "$0: training the diagonal UBM."
  # Use 512 Gaussians in the UBM.
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj $nj \
    --stage -2 \
    --num-frames 500000 --num-threads 1 \
...
if [ $stage -le 4 -a $stop_stage -ge 4 ]; then
  ...
  log "$0: training the iVector extractor"
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj $nj \
    --stage -4 --num-threads 1 --num_processes 1 \
...
if [ $stage -le 5 -a $stop_stage -ge 5 ]; then
  ...
  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" \
    --nj $(min $nj $(cat ${temp_data_root}/${train_set}_sp_hires_max2/spk2utt | wc -l)) \
  ...
  for datadir in $test_sets; do
      steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" \
          --nj $(min $nj $(cat data/${datadir}_hires/spk2utt | wc -l)) \
