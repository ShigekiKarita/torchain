...
if [ $stage -le 2 ] && [ -f data/${train_sp}_hires/feats.scp ]; then
  echo "$0: data/${train_sp}_hires/feats.scp already exists."
...
if [ $stage -le 3 -a $stop_stage -ge 3 ]; then
  ...
  steps/online/nnet2/get_pca_transform.sh --cmd "$train_cmd" \
      ...
      --max-utts 10000 --subsample 2 \
       ${temp_data_root}/${train_sp}_hires_subset \
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
    ...
    ${temp_data_root}/${train_sp}_hires_subset 512 \
...
if [ $stage -le 4 -a $stop_stage -ge 4 ]; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    data/${train_sp}_hires exp/nnet3${nnet3_affix}/diag_ubm exp/nnet3${nnet3_affix}/extractor || exit 1;
fi
...
if [ $stage -le 5 -a $stop_stage -ge 5 ]; then
  ...
  ivectordir=exp/nnet3${nnet3_affix}/ivectors_${train_sp}_hires
  ...
  utils/data/modify_speaker_info.sh --utts-per-spk-max 2 \
    data/${train_sp}_hires ${temp_data_root}/${train_sp}_hires_max2

  steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $nj \
    ${temp_data_root}/${train_sp}_hires_max2 \
...
if [ -f data/${train_sp}/feats.scp ] && [ $stage -le 7 ]; then
  echo "$0: data/${train_sp}/feats.scp already exists.  Refusing to overwrite the features "
  echo " to avoid wasting time.  Please remove the file and continue if you really mean this."
  exit 1;
fi


if [ $stage -le 6 -a $stop_stage -ge 6 ]; then
  echo "$0: preparing directory for low-resolution speed-perturbed data (for alignment)"
  utils/data/perturb_data_dir_speed_3way.sh \
    data/${train_set} data/${train_sp}
fi

if [ $stage -le 7 -a $stop_stage -ge 7 ] && $use_speed_perturb; then
  echo "$0: making MFCC features for low-resolution speed-perturbed data (needed for alignments)"
  steps/make_mfcc.sh --nj $nj \
    --cmd "$train_cmd" data/${train_sp}
...
if [ $stage -le 8 -a $stop_stage -ge 8 ]; then
  ...
  steps/align_fmllr.sh --nj $nj --cmd "$train_cmd" \
    data/${train_sp} data/lang $gmm_dir $ali_dir
fi
