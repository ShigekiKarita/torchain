...
if __name__ == "__main__":
    ...
    get_egs = torchain.GetEgs(
        feat="ark,s,cs:" + cwd + "/res/mfcc.ark",
        supervision="ark:" + cwd + "/res/supervision.ark",
        normalization_fst=cwd + "/res/normalization.fst"
    )
    egs = get_egs.load("1028-20100710-hne-ar-01")
    y = egs[0].outputs["output"]
    x = egs[0].inputs["input"]  # (160, 80)
    print(x)
    ...
    model.cuda()
    # TODO subsample 1/3
    pred = model(x.cuda())[:y.frames_per_sequence]
    print(pred.shape, y)
    loss, result = torchain.chain_loss(pred, y, den_fst)
    print(result)
    loss.backward()
